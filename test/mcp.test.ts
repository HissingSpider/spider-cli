...
check('the timeout is reported as such', /timed out/.test(byName('wedged').error ?? ''),
  byName('wedged').error);
check('a disabled server is not connected', byName('off').state === 'disabled');
check('stderr from the server is kept',
...
check('a readOnlyHint tool runs unprompted', decide(peek, base, 'default', process.cwd()).kind === 'allow');
check('a readOnlyHint tool works while planning', decide(peek, base, 'plan', process.cwd()).kind === 'allow');
check('an un-hinted tool still asks', decide(mutate, base, 'default', process.cwd()).kind === 'ask');
check('an un-hinted tool is refused while planning',
...
check('the change was announced', notices.some((n) => n.includes('updated its tools')),
...
check('excluded tools are not exposed', !('mcp__rich__mutate' in filtered.tools),
  Object.keys(filtered.tools).join(','));
check('the rest are', 'mcp__rich__peek' in filtered.tools);
check('the count of what was dropped is reported',
  filtered.status[0].filtered === 3, String(filtered.status[0].filtered));
await filtered.close();

const only = await connectServers({ rich: { ...rich, tools: ['peek'] } }, process.cwd());
check('an allowlist exposes only what it names',
  Object.keys(only.tools).filter((k) => k.startsWith('mcp__rich__')).length === 1);
await only.close();

console.log('\ntextOf');
check('a bare object is stringified', textOf({ foo: 1 }) === '{"foo":1}');
check('embedded text resources are inlined',
  textOf({ content: [{ type: 'resource', resource: { text: 'inline', uri: 'x' } }] }) === 'inline');

console.log('\ntrust gate');
const { gateUntrusted, fingerprint, isTrusted, trust, untrust } = await import('../src/mcp/trust.ts');
...
check('trust is keyed to what was approved, not the name',
  !isTrusted('__test_srv', cfgB),
  fingerprint(cfgA) + ' vs ' + fingerprint(cfgB));

const gated = await gateUntrusted(
  { __test_srv: cfgA, __test_other: cfgB },
  { interactive: false },
);
...
check('a learned rule lands in settings.local.json',
  fsMod.existsSync(pathMod.join(tmp, '.spider/settings.local.json')));
check('it is not written to the shared project file',
  !fsMod.existsSync(pathMod.join(tmp, '.spider/settings.json')));
check('the local file is git-ignored',
  fsMod.readFileSync(pathMod.join(tmp, '.spider/.gitignore'), 'utf8').includes('settings.local.json'));
