...
check(
  'the message says how to configure it',
  result.output.includes('BRAVE_API_KEY') || result.output.includes('settings.json'),
  result.output,
);
check('it tells the model what to do instead', result.output.includes('web_fetch'), result.output);
check(
  'the tool description warns the model up front',
  unset.spec.description.includes('NOT CONFIGURED'),
  unset.spec.description,
);

const configured = createSearchTool({ provider: 'brave', apiKey: 'test-key' });
check(
  'a configured tool describes itself normally',
  !configured.spec.description.includes('NOT CONFIGURED'),
  configured.spec.description,
);

const empty = await configured.run({ query: '   ' }, process.cwd());
check(
  'an empty query is rejected before any network call',
  empty.isError && empty.output.includes('needs a query'),
  empty.output,
);
...
check(
  'a key in the environment is picked up',
