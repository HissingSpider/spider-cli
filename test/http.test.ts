...
// --- throwIfErrorBody ------------------------------------------------------
check('a clean body passes through', (() => {
  throwIfErrorBody({ id: 'resp_1', output: [] });
  return true;
})());

check('an error object throws', /not allowed/.test(
  (() => { try { throwIfErrorBody({ error: { code: 400, message: 'Sub-product x is not allowed' } }); } catch (e: any) { return e.message; } return ''; })(),
));

check('the error code is preserved', (() => {
  try { throwIfErrorBody({ error: { code: 429, message: 'slow down' } }); } catch (e: any) { return e.code === 429; }
  return false;
})());

check('a string error throws', (() => {
  try { throwIfErrorBody({ error: 'plain string failure' }); } catch (e: any) { return /plain string failure/.test(e.message); }
  return false;
})());

check('a FastAPI detail throws', (() => {
  try { throwIfErrorBody({ detail: 'Missing Authorization header' }); } catch (e: any) { return /Missing Authorization/.test(e.message); }
  return false;
})());

check('null and non-objects are safe', (() => {
  throwIfErrorBody(null); throwIfErrorBody(undefined); throwIfErrorBody('text'); return true;
})());
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/sse-rejected') return json(200, { error: { code: 400, message: 'model not entitled' } });
...
check('HTTP 200 with an error body still throws', /gateway said no/.test(err200 ?? ''), String(err200));

const html = await threw(() => postJSON(base + '/html-403', {}, {}));
check('an HTML error page gives a readable message',
  /Non-JSON response/.test(html ?? '') && /403/.test(html ?? ''), String(html));

const retried = await postJSON(base + '/flaky', {}, {});
check('a 503 is retried until it succeeds', retried.id === 'resp_after_retry', JSON.stringify(retried));
...
check('SSE frames are parsed in order', events.length === 2 && events[0].n === 1 && events[1].n === 2,
  JSON.stringify(events));
check('[DONE] ends the stream', !events.some((e) => e.type === 'after-done'));

const sseErr = await threw(async () => {
  for await (const _ of postSSE(base + '/sse-rejected', {}, {})) { /* drain */ }
});
check('a non-stream error body is surfaced, not parsed as SSE',
  /model not entitled/.test(sseErr ?? ''), String(sseErr));
check('it is a SpiderAIError', await (async () => {
  try {
    for await (const _ of postSSE(base + '/sse-rejected', {}, {})) { /* drain */ }
  } catch (e) {
