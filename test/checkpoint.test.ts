...
check(
  'cp2 captured A as it was at cp2',
  cp2.files.get(fileA) === 'MODIFIED A',
  String(cp2.files.get(fileA)),
);
check(
  'cp1 still holds the original A',
  cp1.files.get(fileA) === 'ORIGINAL A',
...
check(
  'backup with no checkpoints is a no-op',
  (() => {
...
check(
  'listing shows changed-file counts',
  /1 file changed since/.test(
    (() => {
