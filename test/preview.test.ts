...
check(
  'edit_file produces a diff',
  !!ed && ed.includes('-const b = 2;') && ed.includes('+const b = 22;'),
  JSON.stringify(ed),
);
...
check(
  'a non-matching edit yields no diff rather than a wrong one',
  editDiff(noMatch, dir) === null,
);

const newFile: ToolCall = {
  id: 'w',
  name: 'write_file',
  input: { path: 'new.ts', content: 'line one\nline two' },
};
const nd = editDiff(newFile, dir);
check(
  'a new file shows every line as an addition',
  !!nd && nd.every((l) => l.startsWith('+') || l.startsWith('@@')),
  JSON.stringify(nd),
);

fs.writeFileSync(path.join(dir, 'bin.dat'), Buffer.from([1, 0, 2, 0]));
check(
  'a binary file is not diffed',
...
check(
  'preview includes the diff body',
...
check('a server wildcard does not cover another server', !matchesRule('mcp__other__*', mcpCall));
check('an un-hinted MCP tool still asks', decide(mcpCall, base, 'default', dir).kind === 'ask');
check(
  'an un-hinted MCP tool is refused while planning',
  decide(mcpCall, base, 'plan', dir).kind === 'deny',
);

registerReadOnlyTools(['mcp__deerdawn__recall']);
check(
  'a read-only-hinted MCP tool runs unprompted',
  decide(mcpCall, base, 'default', dir).kind === 'allow',
);
check(
  'a read-only-hinted MCP tool is usable while planning (card #22)',
